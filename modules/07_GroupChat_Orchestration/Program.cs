...
Console.WriteLine("Welcome to the Group Chat Triage!");
Console.WriteLine("Four agents (Planner, Investigator, Critic, Scribe) collaborate to triage failures.");
Console.WriteLine("Only the Investigator may call tools.");
Console.WriteLine();
...
while (true)
{
    ...
    Console.WriteLine();
    Console.WriteLine("🚀 Starting group chat triage...");
    Console.WriteLine();

    try
    {
        var (finalText, card) = await GroupChatWorkflow.RunAsync(config, failureReport, logFileName, kbQuery);
