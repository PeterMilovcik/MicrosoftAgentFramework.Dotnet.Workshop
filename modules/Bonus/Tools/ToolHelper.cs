...
/// <summary>
/// Shared Save / Load helpers for the tool classes that manage game entities.
/// Eliminates the repeated try → deserialise → validate-id → store / retrieve → serialise
/// pattern from <see cref="LocationTools"/>, <see cref="NPCTools"/>, and <see cref="CreatureTools"/>.
/// </summary>
internal static class ToolHelper
{
        ...
        string typeName) where T : class, IEntity
    {
        try
        {
            var entity = JsonSerializer.Deserialize<T>(json, AgentHelper.JsonOpts);
        ...
        string typeName) where T : class
    {
        try
        {
            ...
            {
                return JsonSerializer.Serialize(entity, AgentHelper.JsonOpts);
            }
